...
UDP_PORT    = 5005
RECORD_SECS = 4
OUTPUT_FILE = _os.path.join(_os.path.dirname(__file__), '..', '..', 'data', 'lying_data.jsonl')
...
def save_sample(window, idx):
    ...
    print(f"  Saved lying sample #{idx} ({len(window)} packets) to {OUTPUT_FILE}")
...
def main():
    ...
    print("\nLying down data collector ready.")
    print(f"Lie down near the ESP32, then press Enter to record {RECORD_SECS} seconds.")
    print("Press Ctrl+C to stop.\n")

    idx = 1
    while True:
        input(f"[Sample {idx}] Lie down, then press Enter to start recording...")
