...
def save_sample(window, idx):
    ...
    print(f"  Saved walking sample #{idx} ({len(window)} packets) to {OUTPUT_FILE}")
...
def main():
    ...
    print("\nWalking data collector ready.")
    print(f"Walk around near the ESP32, then press Enter to record {RECORD_SECS} seconds.")
    print("Press Ctrl+C to stop.\n")

    idx = 1
    while True:
        input(f"[Sample {idx}] Start walking, then press Enter to start recording...")
